...
# Translator removido - googletrans não é mais usado
import aiofiles
import math
import aiohttp
...
# --- Sistema de Ban Automático para Bots ---
# IDs dos usuários autorizados a controlar o sistema
AUTHORIZED_USERS = [983196900910039090, 1248472513600688229, 978375246623162418]
BANBOT_CHANNEL_ID = 1319063134601871421

# Estado do sistema de ban automático (inicialmente desativado)
auto_ban_bots_enabled = False

def init_banbot_table():
    """Inicializar tabela para configuração do banbot"""
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
    
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS banbot_config (
    ...
    cursor.execute('SELECT COUNT(*) FROM banbot_config')
    if cursor.fetchone()[0] == 0:
        cursor.execute('INSERT INTO banbot_config (id, enabled) VALUES (1, 0)')
    
    conn.commit()
    conn.close()

def get_banbot_status():
    """Obter status atual do sistema de ban automático"""
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
    cursor.execute('SELECT enabled FROM banbot_config WHERE id = 1')
    result = cursor.fetchone()
    conn.close()
    return bool(result[0]) if result else False

def set_banbot_status(enabled: bool):
    """Definir status do sistema de ban automático"""
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE banbot_config SET enabled = ? WHERE id = 1', (int(enabled),))
    conn.commit()
    conn.close()

# Inicializar tabela do banbot
init_banbot_table()

# Sistema de tradução simulado
def simulate_translation(text, target_lang='pt'):
    """Simulador de tradução simples"""
    translations = {
        'hello': 'olá',
    ...
    for english, portuguese in translations.items():
        if english in text_lower:
            text_lower = text_lower.replace(english, portuguese)
...
async def on_member_join(member):
    ...
    # Verificar se o membro é um bot e se o sistema está ativado
    if member.bot and get_banbot_status():
        try:
            # Tentar banir o bot
            await member.ban(reason="Ban automático - Bot não autorizado")
            
            # Enviar notificação no canal específico
            channel = bot.get_channel(BANBOT_CHANNEL_ID)
            if channel:
                embed = discord.Embed(
                    title="🚫 **BOT BANIDO AUTOMATICAMENTE**",
                    description=f"**Bot Banido:** {member.name} ({member.mention})\n**ID:** `{member.id}`\n**Motivo:** Sistema de ban automático ativado",
                    color=discord.Color.red()
                )
                embed.add_field(
                    name="📊 **Informações do Bot**",
                    value=f"**Nome:** {member.name}\n**Tag:** {member.discriminator}\n**Criado em:** {member.created_at.strftime('%d/%m/%Y %H:%M')}\n**Avatar:** [Link]({member.display_avatar.url})",
                    inline=False
                )
                embed.set_thumbnail(url=member.display_avatar.url)
                embed.set_footer(text=f"Sistema de proteção ativo - Dev: YevgennyMXP")
                embed.timestamp = discord.utils.utcnow()
                
                await channel.send(embed=embed)
                
        except discord.Forbidden:
            # Se não tiver permissão para banir
            channel = bot.get_channel(BANBOT_CHANNEL_ID)
            if channel:
                embed = discord.Embed(
                    title="❌ **ERRO AO BANIR BOT**",
                    description=f"**Bot:** {member.name} ({member.mention})\n**Erro:** Sem permissão para banir\n**ID:** `{member.id}`",
                    color=discord.Color.orange()
                )
                embed.set_footer(text="Verifique as permissões do bot - Dev: YevgennyMXP")
                await channel.send(embed=embed)
            ...
            if channel:
                embed = discord.Embed(
                    title="⚠️ **ERRO NO SISTEMA DE BAN**",
                    description=f"**Bot:** {member.name}\n**Erro:** {str(e)}\n**ID:** `{member.id}`",
                    color=discord.Color.dark_red()
                )
                embed.set_footer(text="Erro no sistema automático - Dev: YevgennyMXP")
                await channel.send(embed=embed)
...
class HelpView(View):
    ...
    def get_main_embed(self):
        embed = discord.Embed(
            title="🎯 Central de Comandos - Puerto BOT",
...
async def traduzir(ctx, *, texto):
    try:
        # Usar simulação de tradução
        translated_text = simulate_translation(texto)
        
        embed = discord.Embed(
            title="🌐 Tradução Simulada",
            description=f"**Original:** {texto}\n**Tradução (pt):** {translated_text}",
            color=discord.Color.green()
        )
        embed.set_footer(text="Sistema de tradução simulado - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)
    except Exception as e:
...
@bot.command(name='31231412414141', aliases=['-9182313132', '93198381'])
async def box_itens(ctx):
    """Abrir uma Box de Itens"""
    await create_box_animation(ctx, "itens", "Box de Itens")

@bot.command(name='bo14141414141x-1', aliases=['31341414', '412214142141'])
...
# --- Sistema de Ajuda Profissional ---
class AjudaView(View):
    def __init__(self, user_id: int):
        super().__init__(timeout=300)
        self.user_id = user_id
        self.current_category = "principal"
        
        # Configurar select menu
        self.category_select = Select(
            placeholder="📚 Selecione uma categoria de comandos...",
            options=[
                discord.SelectOption(
                    label="⚽ Carreira e Rolls", 
                    value="carreira",
                    description="Comandos para gerenciar carreira e habilidades"
                ),
                discord.SelectOption(
                    label="💰 Sistema de Economia", 
                    value="economia",
                    description="Comandos de dinheiro, trabalho e apostas"
                ),
                discord.SelectOption(
                    label="🛠️ Moderação", 
                    value="moderacao",
                    description="Ferramentas para moderadores"
                ),
                discord.SelectOption(
                    label="🎮 Diversão e Jogos", 
                    value="diversao",
                    description="Comandos de entretenimento e minijogos"
                ),
                discord.SelectOption(
                    label="📋 Utilitários", 
                    value="utilitarios",
                    description="Ferramentas úteis e informações"
                ),
                discord.SelectOption(
                    label="⚽ Simulações", 
                    value="simulacoes",
                    description="Simuladores de partidas e competições"
                ),
                discord.SelectOption(
                    label="🎁 Boxes e Sistemas", 
                    value="boxes",
                    description="Boxes, parcerias e sistemas especiais"
                ),
                discord.SelectOption(
    ...
    async def category_callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ Apenas quem executou o comando pode usar este menu!", ephemeral=True)
            return
        
        category = interaction.data['values'][0]
        embed = self.get_category_embed(category, interaction.user)
        await interaction.response.edit_message(embed=embed, view=self)
    
    def get_category_embed(self, category: str, user):
        if category == "principal":
            return self.get_main_embed(user)
        elif category == "carreira":
            return self.get_carreira_embed(user)
        elif category == "economia":
            return self.get_economia_embed(user)
        elif category == "moderacao":
            return self.get_moderacao_embed(user)
        elif category == "diversao":
            return self.get_diversao_embed(user)
        elif category == "utilitarios":
            return self.get_utilitarios_embed(user)
        elif category == "simulacoes":
            return self.get_simulacoes_embed(user)
        elif category == "boxes":
            return self.get_boxes_embed(user)
        else:
            return self.get_main_embed(user)
    
    def get_main_embed(self, user):
        embed = discord.Embed(
            title="📖 Central de Ajuda - Puerto BOT",
            description="**Sistema de comandos completo do servidor**\n\nUtilize o menu abaixo para navegar pelas categorias de comandos disponíveis.",
            color=discord.Color.from_rgb(88, 101, 242)
        )
        
        embed.add_field(
            name="📊 Resumo Geral",
            value="• **+80 comandos** disponíveis\n• **8 categorias** organizadas\n• Prefixo padrão: `p!`\n• Sistema atualizado constantemente",
            inline=False
        )
        
        embed.add_field(
            name="🔗 Categorias Principais",
            value="⚽ **Carreira e Rolls** - Gestão de jogador\n💰 **Economia** - Sistema financeiro\n🛠️ **Moderação** - Ferramentas administrativas\n🎮 **Diversão** - Entretenimento e jogos\n📋 **Utilitários** - Ferramentas práticas\n⚽ **Simulações** - Partidas e competições\n🎁 **Boxes** - Sistemas especiais",
            inline=False
        )
        
        embed.set_footer(text=f"Use o menu acima para explorar • Solicitado por {user.display_name}")
        embed.set_thumbnail(url=user.display_avatar.url)
        
        return embed
    
    def get_carreira_embed(self, user):
        embed = discord.Embed(
            title="⚽ Carreira e Rolls - Sistema de Jogador",
            description="**Comandos para gestão completa da sua carreira virtual**",
            color=discord.Color.green()
        )
        
        embed.add_field(
            name="🏆 Gestão de Carreira",
            value="`p!carreira [@usuário]` - Visualizar carreira completa\n`p!alterar <campo> <valor>` - Modificar dados da carreira\n`p!ranking` - Rankings dos melhores jogadores\n`p!ranking_money` - Ranking dos mais ricos",
            inline=False
        )
        
        embed.add_field(
            name="🎲 Sistema de Rolls",
            value="`p!rolls [@usuário]` - Visualizar rolls de habilidades\n`p!editar <roll> <valor>` - Editar seus rolls\n`p!nivel [@usuário]` - Sistema de níveis e XP",
            inline=False
        )
        
        embed.add_field(
            name="📈 Campos Válidos para Carreira",
            value="**Básicos:** nome, nacionalidade, posição, clube\n**Estatísticas:** gols, assistências, desarmes, defesas\n**Títulos:** brasileirão, libertadores, mundial, copa do mundo\n**Atributos:** fintas, perna boa, promessa, físico",
            inline=False
        )
        
        embed.set_footer(text=f"⚽ Use p!carreira para começar • {user.display_name}")
        
        return embed
    
    def get_economia_embed(self, user):
        embed = discord.Embed(
            title="💰 Sistema de Economia - Gestão Financeira",
            description="**Sistema completo de moedas, trabalho e investimentos**",
            color=discord.Color.gold()
        )
        
        embed.add_field(
            name="💵 Gerenciamento Básico",
            value="`p!money [@usuário]` - Verificar saldo\n`p!pay <@usuário> <valor>` - Transferir dinheiro\n`p!ranking_money` - Ranking dos mais ricos",
            inline=False
        )
        
        embed.add_field(
            name="💼 Formas de Ganhar Dinheiro",
            value="`p!daily` - Bônus diário (100-300 moedas + streak)\n`p!work` - Trabalhar por dinheiro (cooldown 1h)\n`p!steal <@usuário>` - Tentar roubar (arriscado)",
            inline=False
        )
        
        embed.add_field(
            name="🎰 Jogos e Investimentos",
            value="`p!apostar <valor>` - Fortune Tiger (caça-níqueis)\n`p!investir <valor>` - Investir em ações/crypto\n`p!odd` - Sistema de apostas esportivas\n`p!duelo <@usuário> <aposta>` - Duelo entre usuários",
            inline=False
        )
        
        embed.add_field(
            name="🛒 Sistema de Compras",
            value="`p!loja` - Ver itens disponíveis\n`p!buy <item>` - Comprar itens da loja",
            inline=False
        )
        
        embed.set_footer(text=f"💰 Use p!daily para começar • {user.display_name}")
        
        return embed
    
    def get_moderacao_embed(self, user):
        embed = discord.Embed(
            title="🛠️ Ferramentas de Moderação",
            description="**Comandos administrativos para manter a ordem**\n*Requer permissões específicas*",
            color=discord.Color.red()
        )
        
        embed.add_field(
            name="🔨 Punições Principais",
            value="`p!ban <@usuário> [motivo]` - Banir permanentemente\n`p!kick <@usuário> [motivo]` - Expulsar do servidor\n`p!mute <@usuário> [tempo] [motivo]` - Silenciar temporariamente\n`p!unmute <@usuário>` - Remover silenciamento",
            inline=False
        )
        
        embed.add_field(
            name="⚠️ Sistema de Avisos",
            value="`p!warn <@usuário> <motivo>` - Dar aviso formal\n`p!warnings [@usuário]` - Ver histórico de avisos",
            inline=False
        )
        ...
        embed.add_field(
            ...
            value="`p!addxp <@usuário> <valor>` - Adicionar XP (admin)\n`p!backup` - Fazer backup dos dados (admin)",
            inline=False
        )
        
        embed.set_footer(text=f"🛠️ Comandos requerem permissões • {user.display_name}")
        
        return embed
    
    def get_diversao_embed(self, user):
        embed = discord.Embed(
            title="🎮 Diversão e Jogos - Entretenimento",
            description="**Comandos para diversão e interação social**",
            color=discord.Color.purple()
        )
        
        embed.add_field(
            name="🎲 Jogos Rápidos",
            value="`p!roll [lados]` - Rolar dado personalizado\n`p!coinflip` - Cara ou coroa clássico\n`p!random <min> <max>` - Número aleatório\n`p!customroll <XdY>` - Dados RPG (ex: 3d6)",
            inline=False
        )
        
        embed.add_field(
            name="🎯 Jogos Interativos",
            value="`p!8ball <pergunta>` - Bola mágica 8\n`p!guess` - Jogo de adivinhação\n`p!poll <pergunta> <opções>` - Criar enquete\n`p!penal <@usuário>` - Duelo de pênaltis",
            inline=False
        )
        
        embed.add_field(
            name="🖼️ Perfil e Avatares",
            value="`p!avatar [@usuário]` - Avatar em alta qualidade\n`p!banner [@usuário]` - Banner do perfil\n`p!color <cor>` - Alterar cor do perfil",
            inline=False
        )
        
        embed.add_field(
            name="🎪 Diversos",
            value="`p!meme` - Meme aleatório\n`p!quote` - Citação inspiradora\n`p!luck` - Medidor de sorte\n`p!word` - Palavra do dia",
            inline=False
        )
        
        embed.set_footer(text=f"🎮 Diversão garantida • {user.display_name}")
        
        return embed
    
    def get_utilitarios_embed(self, user):
        embed = discord.Embed(
            title="📋 Utilitários e Ferramentas",
            description="**Comandos práticos para organização e informações**",
            color=discord.Color.blue()
        )
        
        embed.add_field(
            name="👥 Informações de Usuário",
            value="`p!userinfo [@usuário]` - Perfil detalhado\n`p!serverinfo` - Estatísticas do servidor\n`p!age <ano> [mês] [dia]` - Calculadora de idade",
            inline=False
        )
        
        embed.add_field(
            name="📝 Sistema de Tarefas",
            value="`p!tasks` - Ver tarefas pendentes\n`p!addtask <descrição>` - Adicionar tarefa\n`p!completetask <id>` - Marcar como concluída\n`p!deletetask <id>` - Remover tarefa",
            inline=False
        )
        
        embed.add_field(
            name="🌐 Ferramentas Diversas",
            value="`p!clima_avancado <cidade>` - Clima detalhado\n`p!traduzir <texto>` - Tradutor automático\n`p!calc <expressão>` - Calculadora\n`p!password [tamanho]` - Gerador de senhas",
            inline=False
        )
        
        embed.add_field(
            name="⏰ Tempo e Lembretes",
            value="`p!ping` - Latência do bot\n`p!uptime` - Tempo online\n`p!lembrete <tempo> <texto>` - Criar lembrete\n`p!countdown <segundos>` - Timer",
            inline=False
        )
        
        embed.set_footer(text=f"📋 Ferramentas úteis • {user.display_name}")
        
        return embed
    
    def get_simulacoes_embed(self, user):
        embed = discord.Embed(
            title="⚽ Simulações - Partidas e Competições",
            description="**Simuladores realistas de futebol**",
            color=discord.Color.from_rgb(0, 150, 0)
        )
        
        embed.add_field(
            name="🏟️ Simuladores de Partida",
            value="`p!simular <time1> <time2>` - Partida com transmissão ao vivo\n`p!penaltis <time1> <time2>` - Disputa de pênaltis\n`p!penal <@usuário>` - Duelo 1v1 de pênaltis",
            inline=False
        )
        
        embed.add_field(
            name="🎯 Sistema de Apostas",
            value="`p!odd` - Análise profissional de odds\n`p!historico_apostas` - Ver suas apostas\n`p!apostar <valor>` - Fortune Tiger (slots)",
            inline=False
        )
        
        embed.add_field(
            name="📊 Características das Simulações",
            value="• **Transmissão ao vivo** com comentários\n• **Jogadores reais** da Série A/B\n• **Condições climáticas** realistas\n• **Árbitros famosos** com características\n• **Estatísticas detalhadas** em tempo real",
            inline=False
        )
        
        embed.set_footer(text=f"⚽ Realismo máximo • {user.display_name}")
        
        return embed
    
    def get_boxes_embed(self, user):
        embed = discord.Embed(
            title="🎁 Boxes e Sistemas Especiais",
            description="**Sistemas únicos e recursos especiais**",
            color=discord.Color.from_rgb(255, 215, 0)
        )
        
        embed.add_field(
            name="🎁 Sistema de Boxes",
            value="`p!box-comum` - Abrir box comum\n`p!box-epica` - Abrir box épica\n`p!box-master` - Abrir box master\n`p!box-exclusivas` - Habilidades exclusivas",
            inline=False
        )
        
        embed.add_field(
            name="🤝 Sistema de Parcerias",
            value="`p!parcerias` - Ranking de parcerias\n`p!mparcerias` - Suas parcerias\n*Envie links no canal de parcerias para pontuar*",
            inline=False
        )
        
        embed.add_field(
            name="🏆 Rankings e Estatísticas",
            value="`p!ranking` - Rankings de carreira\n`p!ranking_level` - Ranking de níveis\n`p!botstats` - Estatísticas do bot",
            inline=False
        )
        
        embed.add_field(
            name="🛠️ Outros Sistemas",
            value="`p!feedback <mensagem>` - Enviar feedback\n`p!invite` - Link de convite do bot\n`p!topemojis` - Emojis mais usados",
            inline=False
        )
        
        embed.set_footer(text=f"🎁 Recursos exclusivos • {user.display_name}")
        
        return embed

@bot.command(name='banbot')
async def banbot_command(ctx, status: str = None):
    """Comando para ativar/desativar o ban automático de bots"""
    ...
    if ctx.author.id not in AUTHORIZED_USERS:
        ...
        embed.add_field(
            ...
            value="Apenas usuários específicos podem controlar este sistema.",
            inline=False
        )
        embed.set_footer(text="Sistema de proteção ativo - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)
        return
    
    # Se não forneceu status, mostrar status atual
    if status is None:
        current_status = get_banbot_status()
        status_text = "🟢 **ATIVADO**" if current_status else "🔴 **DESATIVADO**"
        
        embed = discord.Embed(
            title="🤖 **SISTEMA DE BAN AUTOMÁTICO**",
            description=f"**Status Atual:** {status_text}",
            color=discord.Color.green() if current_status else discord.Color.red()
        )
        embed.add_field(
            name="📋 **Como Usar**",
            value="`p!banbot true` - Ativar sistema\n`p!banbot false` - Desativar sistema\n`p!banbot` - Ver status atual",
            inline=False
        )
        embed.add_field(
            name="⚙️ **Funcionamento**",
            value="• Bane automaticamente qualquer bot que entrar\n• Envia notificação no canal configurado\n• Apenas usuários autorizados podem controlar",
            inline=False
        )
        embed.set_footer(text=f"Controlado por {ctx.author.display_name} - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)
        return
    
    # Processar comando de ativação/desativação
    status_lower = status.lower()
    
    if status_lower in ['true', 'on', 'ativar', 'ativo', '1', 'sim']:
        set_banbot_status(True)
        embed = discord.Embed(
            title="✅ **SISTEMA ATIVADO**",
            description="🤖 **Ban automático de bots:** 🟢 **ATIVADO**",
            color=discord.Color.green()
        )
        embed.add_field(
            name="⚡ **Efeito Imediato**",
            value="• Todos os bots que entrarem serão banidos automaticamente\n• Notificações serão enviadas no canal configurado\n• Sistema de proteção ativo",
            inline=False
        )
        embed.set_footer(text=f"Ativado por {ctx.author.display_name} - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)
        ...
        if channel:
            log_embed = discord.Embed(
                title="🟢 **SISTEMA DE BAN ATIVADO**",
                description=f"**Ativado por:** {ctx.author.mention} (`{ctx.author.id}`)\n**Canal:** {ctx.channel.mention}\n**Horário:** {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}",
                color=discord.Color.green()
            )
            log_embed.set_footer(text="Sistema de proteção automática - Dev: YevgennyMXP")
            await channel.send(embed=log_embed)
    
    elif status_lower in ['false', 'off', 'desativar', 'inativo', '0', 'não', 'nao']:
        set_banbot_status(False)
        embed = discord.Embed(
            title="🔴 **SISTEMA DESATIVADO**",
            description="🤖 **Ban automático de bots:** 🔴 **DESATIVADO**",
            color=discord.Color.red()
        )
        embed.add_field(
            name="📋 **Status Atual**",
            value="• Bots podem entrar normalmente no servidor\n• Sistema de proteção desligado\n• Nenhum ban automático será aplicado",
            inline=False
        )
        embed.set_footer(text=f"Desativado por {ctx.author.display_name} - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)
        ...
        if channel:
            log_embed = discord.Embed(
                title="🔴 **SISTEMA DE BAN DESATIVADO**",
                description=f"**Desativado por:** {ctx.author.mention} (`{ctx.author.id}`)\n**Canal:** {ctx.channel.mention}\n**Horário:** {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}",
                color=discord.Color.red()
            )
            log_embed.set_footer(text="Sistema de proteção automática - Dev: YevgennyMXP")
            await channel.send(embed=log_embed)
        ...
        embed.add_field(
            ...
            value="`p!banbot true` - Ativar\n`p!banbot false` - Desativar\n`p!banbot` - Ver status",
            inline=False
        )
        embed.set_footer(text="Use os parâmetros corretos - Dev: YevgennyMXP")
        await ctx.reply(embed=embed)

@bot.command(name='ajuda')
async def ajuda_command(ctx):
    """Sistema de ajuda profissional com menu organizado"""
    
    view = AjudaView(ctx.author.id)
    embed = view.get_main_embed(ctx.author)
